...
import { useChatMutation, useGetChatHistory } from "../api/chats/useChats";
...
export default function TextGeneration() {
  ...
  const { mutate: chatM, isPending: chatIsP } = useChatMutation();
    ...
    <>
      ...
      <div className="flex-1 overflow-y-auto px-2 sm:px-6 py-4 sm:py-6 space-y-4">
        ...
        {messages.map((msg) => (
          <div
            key={msg._id}
            className={`max-w-full my-5 sm:max-w-lg px-4 py-3 rounded-2xl shadow-md shadow-cyan-500/5 break-words ${
              msg.sender === "user"
                ? "bg-cyan-500 text-white self-end ml-auto"
                : "bg-gray-800/60 text-gray-200 backdrop-blur-lg border border-gray-700/50"
            }`}
          >
